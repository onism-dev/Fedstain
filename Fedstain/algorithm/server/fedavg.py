...
def get_fedavg_argparser():
    ...
    parser.add_argument(
        ...
        "--dataset",
        type=str,
        choices=["pacs", "vlcs", "office_home", "domainnet", "minidomainnet", "came", "midog"],
        default="pacs",
    )
    parser.add_argument(
        "--partition_info_dir",
...
class FedAvgServer:
    ...
    def initialize_clients(self):
        ...
        self.client_list = []
        client_id_counter = 0
        for domain_idx, domain in enumerate(ALL_DOMAINS[self.args.dataset]):
            if domain != self.args.test_domain:  # 跳过测试域
                for local_client_id in range(self.args.num_clients_per_domain):
                    # 创建全局唯一的客户端ID
                    global_client_id = client_id_counter  # 使用数字ID
                    dataset = FLDataset(self.args, global_client_id)
                    # 跳过没有样本的客户端，避免 DataLoader 报错
                    if len(dataset) == 0:
                        self.logger.log(f"Skip empty client_id={global_client_id} (domain={domain})")
                        client_id_counter += 1
                        continue
                    client = FedAvgClient(
                        self.args, dataset, global_client_id, self.logger
                    )
