...
from algorithm.client.fedstain import FedStainClient
...
def get_fedstain_argparser():
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.1,
        help="hyper-parameter for representation alignment",
    )
    parser.add_argument(
        "--lambda2",
        type=float,
        default=0.1,
        help="hyper-parameter for prediction alignment",
    )
    parser.add_argument("-t", type=float, default=0.1, help="temperature")
    return parser


class FedStainServer(FedAvgServer):
    def __init__(self, algo="FedStain", args: Namespace = None):
        if args is None:
            args = get_fedstain_argparser().parse_args()
    ...
    def visualize_augmentation_effect(self):
        statistic_pool = self.generate_statistic_pool()
        for client_id in range(self.num_client):
            self.client_list[client_id].download_statistic_pool(
                deepcopy(statistic_pool)
